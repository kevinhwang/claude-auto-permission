...
By default the classifier both auto-approves calls aligned with your session intent and blocks dangerous ones. If you
want the transcript-aware veto without ever letting the classifier approve a call, set
`mode: CLASSIFIER_MODE_BLOCK_ONLY`. E.g.:

```txtpb
llm_classifier {
  enabled: true
  mode: CLASSIFIER_MODE_BLOCK_ONLY
  bedrock {
    model_id: "global.anthropic.claude-sonnet-5"
  }
}
...
| Mode                                    | Behavior                                                                                                                                                        |
|-----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `CLASSIFIER_MODE_FULL_AUTO` *(default)* | Auto-approves aligned calls and blocks dangerous ones — the full auto-mode experience.                                                                          |
| `CLASSIFIER_MODE_BLOCK_ONLY`            | Only ever denies (or asks, on backstop/error). A no-block verdict defers to the static Bash allowlist and Claude Code's normal flow rather than auto-approving. |

The model round-trip is identical in both modes — `block_only` only reinterprets a no-block verdict, so latency and
cost are unchanged. The deny veto (e.g. blocking a `git push` you told the agent to hold off on) works the same either
way.
...
|-----------------------------------|----------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| `TWO_STAGE_MODE_BOTH` *(default)* | Stage 1 is a conservative XML prefilter; on `yes`, stage 2 escalates to chain-of-thought for the authoritative reason. | Best balance of correctness and latency. Matches official Auto Mode's production default.               |
| `TWO_STAGE_MODE_FAST`             | Stage 1 only.                                                                                                        | Fastest. Higher false-positive rate on nuanced cases — no chain-of-thought reasoning about user intent. |
| `TWO_STAGE_MODE_OFF`              | Single forced-tool-use call against a JSON Schema.                                                                   | Legacy. Haiku 4.5 has a known issue with this mode (drops the verdict boolean from structured output).  |
| `TWO_STAGE_MODE_THINKING`         | Single XML chain-of-thought call.                                                                                    | Useful as an A/B baseline against the two-stage modes.                                                  |

Set via `bedrock { two_stage: TWO_STAGE_MODE_FAST }`.
