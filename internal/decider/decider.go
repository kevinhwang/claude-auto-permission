...
type Env struct {
	// Cwd is the hook input's `cwd`: Claude Code's live working directory at the time of the tool call. It tracks the
	// agent's `cd`s within the session, so it can drift away from the launch directory mid-session. For a stable
	// project anchor, use ProjectRoot instead.
	Cwd string
	...
	//
	//   - `permissions.deny` unioned across all tiers; `permissions.additionalDirectories` from trusted tiers only
	//     (the repo-controlled project `settings.json` is excluded — see [permscope]).
