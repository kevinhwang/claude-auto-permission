...
func TestHook_AutoPermissionModeElidesHook(t *testing.T) {
	dec := &stubDecider{name: "static_bash_rules", result: decider.Result{Decision: decider.DecisionAllow}}
	raw, err := json.Marshal(hookio.HookInput{
		HookEventName:  hookio.EventPreToolUse,
		PermissionMode: hookio.PermissionModeAuto,
		ToolName:       "Bash",
	...
	out := runHook(t, raw, dec)
	if out != "" {
		t.Errorf("expected auto permission mode to elide hook output; got %q", out)
	}
	if dec.calls != 0 {
		t.Errorf("expected auto permission mode to skip deciders; got %d calls", dec.calls)
	}
}
