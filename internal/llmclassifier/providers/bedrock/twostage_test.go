...
func TestRewriteSystemForXml(t *testing.T) {
	t.Run("replaces canonical line", func(t *testing.T) {
		// The canonical instruction line lives in the prompt package ([prompt.ClassifyResultLine]); the system template
		// emits it and we substitute it here. A prompt-package test pins that the rendered system prompt still contains it.
		got := rewriteSystemForXml("preamble " + prompt.ClassifyResultLine + " trailer")
		if strings.Contains(got, prompt.ClassifyResultLine) {
