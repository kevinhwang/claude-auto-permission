...
func TestRewriteSystemForXml(t *testing.T) {
	t.Run("keeps XML-native prompt unchanged", func(t *testing.T) {
		in := "preamble\n\n" + prompt.XMLOutputFormat
		if got := rewriteSystemForXml(in); got != in {
			t.Errorf("got rewritten prompt\n--- got ---\n%s\n--- want ---\n%s", got, in)
		}
	})
	t.Run("upgrades forced-tool line", func(t *testing.T) {
		got := rewriteSystemForXml("preamble " + prompt.ForcedToolResultLine + " trailer")
		if strings.Contains(got, prompt.ForcedToolResultLine) {
			t.Error("expected forced-tool line to be replaced")
		}
		if !strings.Contains(got, prompt.XMLOutputFormat) {
	...
	t.Run("appends when canonical missing", func(t *testing.T) {
		...
		if !strings.HasSuffix(got, prompt.XMLOutputFormat) {
			t.Errorf("xml format not appended; tail: %q", got[len(got)-50:])
		}
	})
}

func TestRewriteSystemForForcedToolUse(t *testing.T) {
	t.Run("replaces XML output block", func(t *testing.T) {
		got := rewriteSystemForForcedToolUse("preamble\n\n" + prompt.XMLOutputFormat)
		if strings.Contains(got, prompt.XMLOutputFormat) {
			t.Error("expected XML output block to be replaced")
		}
		if !strings.Contains(got, prompt.ForcedToolResultLine) {
			t.Errorf("missing forced-tool marker; got: %q", got)
		}
	})
	t.Run("keeps forced-tool prompt unchanged", func(t *testing.T) {
		in := "preamble\n\n" + prompt.ForcedToolResultLine
		if got := rewriteSystemForForcedToolUse(in); got != in {
			t.Errorf("got rewritten prompt\n--- got ---\n%s\n--- want ---\n%s", got, in)
		}
	})
	t.Run("appends when output format missing", func(t *testing.T) {
		got := rewriteSystemForForcedToolUse("system without the marker")
		if !strings.HasSuffix(got, prompt.ForcedToolResultLine) {
			t.Errorf("forced-tool marker not appended; tail: %q", got[len(got)-50:])
		}
	})
}
...
func TestProvider_TwoStageFast_ParseFailBlocks(t *testing.T) {
	// Fast mode is final, so no parseable verdict is treated as a block for safety.
	p, mc := providerForMode(t, TwoStageFast)
	mc.EXPECT().
		Converse(gomock.Any(), gomock.Any()).
	...
	if !strings.Contains(res.Reason, "unparseable") {
		t.Errorf("Reason = %q, want unparseable safety block", res.Reason)
	}
}
...
func TestProvider_TwoStageThinking_ParseFailBlocks(t *testing.T) {
	p, mc := providerForMode(t, TwoStageThinking)
	mc.EXPECT().
		Converse(gomock.Any(), gomock.Any()).
		Return(xmlTextOutput("<thinking>ran out of room"), nil)
	...
	if !strings.Contains(res.Reason, "stage 2 unparseable") {
		t.Errorf("Reason = %q, want stage 2 parse safety block", res.Reason)
	}
}
...
func TestStage1Request_HasStopSequence_BothMode(t *testing.T) {
	...
	if !contentContains(captured.Messages[0].Content, xmlS1BothSuffix) {
		t.Error("stage 1 both-mode prompt missing conservative prefilter suffix")
	}
}
...
func TestStage1Request_NoStopSequence_FastMode(t *testing.T) {
	...
	if !contentContains(captured.Messages[0].Content, xmlS1FastSuffix) {
		t.Error("fast-mode prompt missing immediate verdict suffix")
	}
	if contentContains(captured.Messages[0].Content, xmlS1BothSuffix) {
		t.Error("fast-mode prompt should not use the both-mode prefilter suffix")
	}
}
...
func TestStage2Request_IsXML_NotForcedToolUse(t *testing.T) {
	...
	if !contentContains(stage2.Messages[0].Content, xmlS2Suffix) {
		t.Error("stage 2 prompt missing ambiguity guidance suffix")
	}
}
...
func TestSingleStageRequest_OmitsTemperature(t *testing.T) {
	// Models with always-on adaptive thinking (e.g. Claude Sonnet 5) reject Converse calls that set an explicit
	// temperature with a 400. Leaving it unset must work across every model the classifier targets.
	captured := captureSingleStage(t, false)
	if captured.InferenceConfig == nil {
		t.Fatal("InferenceConfig is nil")
	}
	if temp := captured.InferenceConfig.Temperature; temp != nil {
		t.Errorf("Temperature = %v, want unset", *temp)
	}
}
