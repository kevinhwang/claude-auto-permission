...
func (p *Provider) classifyForcedToolUse(ctx context.Context, req providers.Request) (providers.Result, error) {
	...
	if req.SystemPrompt != "" {
		input.System = cachedSystemBlocks(rewriteSystemForForcedToolUse(req.SystemPrompt))
	}
...
func rewriteSystemForForcedToolUse(s string) string {
	if strings.Contains(s, prompt.ForcedToolResultLine) {
		return s
	}
	if strings.Contains(s, prompt.XMLOutputFormat) {
		return strings.Replace(s, prompt.XMLOutputFormat, prompt.ForcedToolResultLine, 1)
	}
...
// BOTH) pass `["</block>"]`.
//
// Temperature is deliberately omitted: models with always-on adaptive thinking (e.g. Claude Sonnet 5) reject any
// explicit value with a 400, and nothing here depends on deterministic sampling — the classifier re-asks on every
// call rather than caching by prompt hash.
