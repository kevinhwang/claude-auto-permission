...
func FromProto(ctx context.Context, cfg *configpb.LlmClassifierConfig) (*Provider, error) {
	...
	return New(ctx, Config{
		...
		Stage1MaxTokensBoth: bp.GetStage1MaxTokensBoth(),
		Stage1MaxTokensFast: bp.GetStage1MaxTokensFast(),
...
// annotation, so by the time this runs `TWO_STAGE_MODE_UNSPECIFIED` should already have been mapped to `TWO_STAGE_MODE_BOTH`; the
// explicit case is defense in depth.
func twoStageMode(m configpb.BedrockProvider_TwoStageMode) TwoStageMode {
	switch m {
	case configpb.BedrockProvider_TWO_STAGE_MODE_OFF:
		return TwoStageOff
	case configpb.BedrockProvider_TWO_STAGE_MODE_FAST:
		return TwoStageFast
	case configpb.BedrockProvider_TWO_STAGE_MODE_BOTH:
		return TwoStageBoth
	case configpb.BedrockProvider_TWO_STAGE_MODE_THINKING:
