...
type classifierCfgOpts struct {
	...
	mode                 *configpb.LlmClassifierConfig_ClassifierMode
}
...
// TestDecide_BlockOnlyMode_NoBlockIsSilent verifies that in block_only mode a no-block verdict is emitted as silent
// (no opinion) rather than allow — the classifier withholds approval authority so peer deciders and Claude Code's
// normal flow decide the call. Meta still surfaces so the decision log attributes the silent vote to its provider.
func TestDecide_BlockOnlyMode_NoBlockIsSilent(t *testing.T) {
	ctrl := gomock.NewController(t)
	prov := newProvider(ctrl)
	prov.EXPECT().Classify(gomock.Any(), gomock.Any()).Return(providers.Result{ShouldBlock: false}, nil)
	mode := configpb.LlmClassifierConfig_CLASSIFIER_MODE_BLOCK_ONLY
	d := newDecider(t, ctrl, prov, toolprep.SkipNone, classifierCfgOpts{mode: &mode})
	in := bashInput(t, "git status", "s1")
	r := d.Decide(context.Background(), in, envFor(in))
	if r.Decision != decider.DecisionSilent {
		t.Errorf("Decision = %q, want silent", r.Decision)
	...
	if r.Reason == "" {
		t.Errorf("block_only silent should carry a reason explaining the no-block downgrade")
	}
	if r.Meta["provider"] != "stub" || r.Meta["model"] != "stub-model" {
		t.Errorf("block_only silent must populate Meta provider/model: %v", r.Meta)
	}
}

// TestDecide_BlockOnlyMode_BlockStillDenies verifies the headline veto survives block_only mode: a block verdict is a
// deny exactly as in full_auto. block_only only reinterprets the no-block verdict, never the block path.
func TestDecide_BlockOnlyMode_BlockStillDenies(t *testing.T) {
	ctrl := gomock.NewController(t)
	prov := newProvider(ctrl)
	prov.EXPECT().Classify(gomock.Any(), gomock.Any()).Return(providers.Result{ShouldBlock: true, Reason: "user said don't push"}, nil)
	mode := configpb.LlmClassifierConfig_CLASSIFIER_MODE_BLOCK_ONLY
	d := newDecider(t, ctrl, prov, toolprep.SkipNone, classifierCfgOpts{mode: &mode})
	in := bashInput(t, "git push origin main", "s1")
	r := d.Decide(context.Background(), in, envFor(in))
	if r.Decision != decider.DecisionDeny {
		t.Errorf("Decision = %q, want deny", r.Decision)
	}
	if r.Reason != "user said don't push" {
		t.Errorf("Reason = %q, want the verbatim block reason", r.Reason)
	}
}
