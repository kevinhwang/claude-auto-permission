...
// the trusted working-dir set (cwd ∪ trusted-tier `permissions.additionalDirectories`; the repo-controlled project
// settings.json is excluded upstream by permscope) and lets Claude Code's normal flow handle the call; out-of-scope
// paths (~/.aws, /etc, …) reach the classifier.
//
