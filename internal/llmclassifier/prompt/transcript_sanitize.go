...
//
//   - Real user prompts (string content, or text blocks in non-meta user content) → kept verbatim.
//   - Queued-command attachments → kept as user turns.
...
//   - Synthetic-user tool_result blocks → dropped (execution state is out of scope; matches reference toCompactBlock).
//   - System-issued user messages (isMeta==true) → dropped (not real intent).
//   - System messages, non-queued-command attachments, queue-ops, *-title records → dropped (framework metadata).
//
...
func sanitizeEntry(e cctranscript.Entry, sanitizeInput ToolInputSanitizer) ([]Record, error) {
	...
	case "attachment":
		return sanitizeAttachmentEntry(e)
	}
	// system / non-queued-command attachment / queue-operation / *-title / file-history are framework metadata.
	return nil, nil
}
...
func sanitizeUserEntry(e cctranscript.Entry) ([]Record, error) {
	...
	text, ok := decodeUserPromptText(e.Raw.Message.Content)
	if !ok {
		return nil, nil
	}
	return []Record{{
...
func decodeUserPromptText(raw json.RawMessage) (string, bool) {
...
func sanitizeAttachmentEntry(e cctranscript.Entry) ([]Record, error) {
	if e.Raw.Attachment.Type != "queued_command" {
		return nil, nil
	}
	text, ok := decodeQueuedCommandPrompt(e.Raw.Attachment.Prompt)
	if !ok {
		return nil, nil
	}
	return []Record{{
...
func decodeQueuedCommandPrompt(raw json.RawMessage) (string, bool) {
	if text, ok := decodeString(raw); ok {
		return text, true
	}
	blocks, err := decodeBlocks(raw)
...
func textBlockParts(blocks []cctranscript.RawBlock) []string {
	var parts []string
	for _, b := range blocks {
