...
// ClassifyResultLine is the structured-output instruction the system template emits as its final line (injected via
// [systemTemplateData] so it lives in one place). The XML two-stage modes locate and replace it with their own
// output-format block, so it's exported as the single source of truth for that swap.
const ClassifyResultLine = "Use the classify_result tool to report your classification."
...
func systemDataFor(in BuildInput) systemTemplateData {
	...
	data := systemTemplateData{
		...
		ClassifyResultLine: ClassifyResultLine,
	}
