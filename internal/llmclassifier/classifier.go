...
//
// Every failure mode maps to the configured unavailable fallback (passthrough by default) with a populated reason —
// "we can't classify right now," not "block the user." The orchestrator's combiner then lets Claude Code's normal
// permission flow handle the call unless the user configured the classifier to ask on unavailability.
...
// buildPrompt loads the auto-mode policy and renders the prompt for this call. The policy load is per-project
// (`claude auto-mode config` reads settings anchored at its launch dir, so we run it from the project root rather than
// the live cwd); the resulting cache key folds in every reachable settings path so different projects don't share
// entries. The cache TTL is a process-level (Runtime) knob, not a per-project one.
func (d *Decider) buildPrompt(ctx context.Context, env decider.Env, records []prompt.Record, proposed prompt.CallRecord) (prompt.BuildOutput, error) {
	ttl := time.Duration(d.cfg.Proto().GetRuntime().GetAutomodePolicyTtlSeconds()) * time.Second
	...
	return d.buildPromptFn(prompt.BuildInput{
		...
		Cwd:          env.Cwd,
		ProjectRoot:  env.ProjectRoot,
		WorkingDirs:  env.PermScope.TrustedWorkingDirs,
