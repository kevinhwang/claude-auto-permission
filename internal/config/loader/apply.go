...
// FillDefaults seeds msg with schema-declared defaults for scalar fields the user left unset. Safe to call on a message
// that already has user-specified values — it won't overwrite them.
//
// Enums are the exception: a field sitting at its zero value (the `*_UNSPECIFIED` sentinel by proto convention) is
// treated as unset and takes its default, since an explicit `UNSPECIFIED` expresses no real choice. This keeps both
// absence and an explicit sentinel resolving through the schema default rather than diverging.
//
...
// needsDefault reports whether fd should take its schema default: true when the field is unset, or when it's an enum
// resting on its zero (`*_UNSPECIFIED`) value — an explicit sentinel that names no real choice.
func needsDefault(container protoreflect.Message, fd protoreflect.FieldDescriptor) bool {
	if !container.Has(fd) {
		return true
	}
	return fd.Kind() == protoreflect.EnumKind && container.Get(fd).Enum() == 0
}
