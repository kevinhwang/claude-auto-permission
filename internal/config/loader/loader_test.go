...
func TestFillDefaults_BedrockMaxTokensDefault(t *testing.T) {
	msg := &configpb.BedrockProvider{}
	if err := FillDefaults(msg); err != nil {
		t.Fatalf("FillDefaults: %v", err)
	}
	if got := msg.GetMaxTokens(); got != 8192 {
		t.Errorf("max_tokens default = %d, want 8192", got)
	}
}
