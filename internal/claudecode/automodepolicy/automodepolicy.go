...
type Loader struct {
	...
	ProjectRoot   string   // cmd.Dir on cache miss
}
...
//
// On a cache miss it shells out to `claude` with `cmd.Dir=l.ProjectRoot`. `claude auto-mode config` resolves
// `settings.local.json` from its own launch dir (which doesn't follow the agent's `cd`s), so the project root is the
// closest stable anchor — running from the live cwd could read a different project's settings.
func (l *Loader) LoadOrDefaults(ctx context.Context) Policy {
	...
	p, err := fetchFromClaude(ctx, binary, l.ProjectRoot)
	if err != nil {
...
// fetchFromClaude shells out to `claude auto-mode config` from the project root so settings.local.json resolves to the
// right project (see [Loader.LoadOrDefaults]).
func fetchFromClaude(ctx context.Context, binary, projectRoot string) (Policy, error) {
