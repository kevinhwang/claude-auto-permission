...
func TestLoad_DistinctCwdsWithMissingSettings(t *testing.T) {
	...
	lA := &Loader{BinaryPath: binary, CacheDir: cacheDir, SettingsPaths: pathsA, ProjectRoot: cwdA, Ttl: time.Hour}
	lB := &Loader{BinaryPath: binary, CacheDir: cacheDir, SettingsPaths: pathsB, ProjectRoot: cwdB, Ttl: time.Hour}
...
func TestLoad_PassesProjectRootToClaude(t *testing.T) {
	...
	d := l.LoadOrDefaults(context.Background())
	if len(d.Allow) != 1 || d.Allow[0] != "cwd="+root {
		t.Errorf("stub did not receive cmd.Dir=projectRoot; got Allow=%v want [cwd=%s]", d.Allow, root)
	}
}
...
func TestBundledDefaults_NonEmpty(t *testing.T) {
	...
	// Spot-check that representative rules parsed into the correct tiers, keyed on stable rule names and the
	// literal markers/values those rules are defined in terms of (not on incidental prose, which is reworded freely).
	assertContainsRule(t, p.SoftDeny, "Live-Shared Artifact Sensitive Delta", "[shared-live:")
	assertContainsRule(t, p.SoftDeny, "Package Registry Bypass", ".npmrc")
	assertContainsRule(t, p.SoftDeny, "Browser Navigate Exfil", "Chrome-MCP")
	assertContainsRule(t, p.Allow, "Browser Trusted Navigation", "localhost")
	assertContainsRule(t, p.Environment, "Internal package registry", "None configured")
}
...
func assertContainsRule(t *testing.T, rules []string, name, fragment string) {
	t.Helper()
	for _, rule := range rules {
		if strings.Contains(rule, name) && strings.Contains(rule, fragment) {
			return
		}
	}
	t.Errorf("expected bundled rule %q containing %q", name, fragment)
}
