...
// ProjectSettingsFiles returns .claude/settings.json and .claude/settings.local.json anchored at the given dir, which
// should be the project root. Claude Code anchors these at its stable launch dir (`getOriginalCwd()`), not the live
// per-`cd` cwd, and does not walk ancestors — so callers pass [Paths.ProjectRoot] (`$CLAUDE_PROJECT_DIR`, else launch
// cwd), the closest stable anchor the hook exposes. (Mid-session worktree entry / cross-project resume can still move
// CC's anchor away from both values the hook surfaces; see the classifier design doc's limitations.)
func (p Paths) ProjectSettingsFiles(dir string) []string {
	if dir == "" {
		return nil
	}
	return []string{
...
// Callers union across all — Claude Code does not short-circuit on first hit. `dir` anchors the project-tier files and
// should be the project root (see [Paths.ProjectSettingsFiles]).
func (p Paths) AllSettingsFiles(dir string) []string {
	var out []string
	out = append(out, p.ManagedSettingsFiles()...)
	if f := p.UserSettingsFile(); f != "" {
		out = append(out, f)
	}
	out = append(out, p.ProjectSettingsFiles(dir)...)
	return out
}
