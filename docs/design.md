...
| `passthrough` | This decider had a stake in the call but couldn't vote (provider outage, transcript read failure, prompt build failure, ...). Beats `allow` because withholding approval defers to Claude Code's own permission flow, which is more cautious than asserting `allow` — so a peer's permissive vote can't fast-path while this one is incapacitated. | (none — Claude Code's normal flow handles the call) |
...
{"ts":"...","session_id":"...","tool":"Bash","cwd":"/proj","input_sha":"...",
 ...
 "deciders":{
   ...
   "llm_classifier":{"decision":"deny","reason":"User explicitly forbade...","latency_ms":1843,
                     "meta":{"provider":"bedrock","model":"global.anthropic.claude-sonnet-4-6"}}}}
```
