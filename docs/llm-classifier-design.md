...
- **Threat-model parity with Anthropic's official Auto Mode.** The classifier sees only the user's verbatim messages
  and the agent's tool calls — never assistant prose, reasoning, or tool outputs. This is the load-bearing safety
  property; everything else flows from it.
...
- **Post-hoc subagent return review.** Subagent spawn time is covered (the spawn itself classifies); per-action calls
  inside the subagent are also covered. Reviewing the subagent's full action history *after* it returns — useful when
  a prompt-injected subagent returns text that then induces unsafe parent behavior — is out of scope.
...
The Claude Code source ([public mirror](https://github.com/codeaashu/claude-code)) is the authoritative behavioral
reference. Where this design preserves a property of Auto Mode, the relevant source location is linked in the
...
| **No prompt-injection probe**                       | Auto Mode runs an additional server-side prompt-injection probe inside Anthropic's inference path. Replicating that from a hook is hard — Anthropic's prompts, corpus, and tuning aren't public. See [Limitations](#limitations--known-gaps). |
...
- **Fail closed on runaway block streaks.** After N consecutive blocks or M total per session (defaults: 3 / 20,
  ...
  misconfiguration or a stuck agent.
...
|------------------------------|------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| `TWO_STAGE_BOTH` *(default)* | Tight max-tokens budget; XML `<block>yes/no</block>`. Stops as soon as the verdict tag closes. | On stage-1 `yes`, escalate to a chain-of-thought call for the authoritative reason.            | Best balance. Allow path is fast (no stage 2); deny path pays for reasoning. Matches Auto Mode's production default.                    |
| `TWO_STAGE_FAST`             | Same as Both, slightly bigger token budget for inline reason.                                  | Skipped.                                                                                       | Lowest latency. Higher false-positive rate on nuanced cases — no chain-of-thought to reason about user intent.                          |
| `TWO_STAGE_OFF`              | Skipped.                                                                                       | Single forced-tool-use call against a JSON Schema requiring `{thinking, shouldBlock, reason}`. | Strongest structured-output guarantees, slower. Some models (Haiku 4.5) drop the verdict boolean from forced tool use; treat as legacy. |
| `TWO_STAGE_THINKING`         | Skipped.                                                                                       | Single XML chain-of-thought call.                                                              | Useful as an A/B baseline against the two-stage modes.                                                                                  |
...
- **Forced tool use against a JSON Schema** (`TWO_STAGE_OFF` mode): the schema requires `thinking` before
...
- Allow paths in `TWO_STAGE_BOTH` are fastest — Haiku models land well under a second, Sonnet is roughly 2× slower.
- `TWO_STAGE_FAST` is similarly fast on both verdicts (no stage 2).
- `TWO_STAGE_OFF` is several times slower on the allow path — chain-of-thought scratch space costs output tokens.
- `TWO_STAGE_BOTH` block paths are the slowest — stage 2's chain-of-thought generates the bulk of output tokens.
...
2. **No subagent return-check.** Subagent spawn and per-action calls are covered. Reviewing the subagent's full action
   history *after* it returns — useful when a prompt-injected subagent returns text that then induces unsafe parent
...
### Follow-Up: Prompt-Injection Probe

Anthropic's official Auto Mode includes a server-side prompt-injection probe — an additional check that runs inside
their inference path, with prompts and a corpus tuned by the Anthropic team. We'd like to add a local equivalent
eventually (likely as a `PostToolUse` hook that inspects tool outputs for injection patterns), but it's hard:
Anthropic's prompts and tuning corpus aren't public, so we can't simply port their work, and a probe with a
significantly worse FPR/FNR profile than theirs would erode user trust faster than its catches earn it.

Current posture: we don't claim parity with Auto Mode on this dimension. Defense 1 (transcript stripping) is the
primary mitigation for indirect prompt injection from tool outputs into the classifier itself; it doesn't address
injection from tool outputs back into the *agent*, which is what the probe is for.
...
[Public Claude Code mirror](https://github.com/codeaashu/claude-code) — authoritative behavioral reference. Relevant
locations under `src/`:
