...
Properties this design preserves faithfully:
...
| **Structured output**                                        | The verdict carries `{thinking, shouldBlock, reason}` — JSON via forced tool use, or XML tags in the two-stage modes.                                                                                                                              |

Properties this design deviates from, deliberately:

| Difference                                          | Why                                                                                                                                                                                                                             |
|-----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Auto-mode policy resolved at runtime**            | Anthropic bakes Auto Mode's policy into the `claude` binary as compile-time strings. We shell out to the user's installed `claude` binary at runtime — no version skew, automatic upgrades — with a bundled baseline only as fallback. |
...
- Queued user prompts (`queued_command` attachments drained mid-turn).
- User answers surfaced through `AskUserQuestion` text blocks.
...
- **Withhold on infrastructure failure.** Provider timeout, forced-tool-use parse error, transcript read error, prompt build error —
...
| **In-scope file ops**            | `Read`, `Write`, `Edit`, `NotebookEdit`                                 | Target path resolves under cwd or a trusted-tier `permissions.additionalDirectories` (the repo-controlled project `settings.json` is excluded). | Skip — let Claude Code decide via its session state (which knows about acceptEdits, mid-session "always allow this session" UI grants, etc.). |
...
|-----------------------------------|------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| `TWO_STAGE_MODE_BOTH` *(default)* | Tight max-tokens conservative prefilter; XML `<block>yes/no</block>`. Stops as soon as the verdict tag closes. | On stage-1 `yes`, escalate to a chain-of-thought call that applies the full classification process. | Best balance. Allow path is fast (no stage 2); deny path pays for reasoning. Matches Auto Mode's production default.                    |
| `TWO_STAGE_MODE_FAST`             | Final XML verdict with a slightly bigger token budget for inline reason.                       | Skipped.                                                                                       | Lowest latency. Higher false-positive rate on nuanced cases — no chain-of-thought to reason about user intent.                          |
...
- **Static scaffold.** Context, threat model, default rule, user-intent rule, evaluation rules, classification process,
  output format. Modeled on Auto Mode's classifier prompt — same structure and decision semantics.
...
- **Environment block.** Trust bullets from the policy (trusted repo, trusted internal domains, key services, …),
  plus a Working directories sub-bullet listing cwd and `permissions.additionalDirectories`. The latter is **a
  deviation from the official Auto Mode**: it teaches the classifier not to false-positive on dirs the user explicitly
  added to scope. The additional-directories come from trusted tiers only — the repo-controlled project
  `<cwd>/.claude/settings.json` is excluded (see [Trust Tiers](#trust-tiers-across-the-parts)), and cwd is rendered as
  the agent's current location rather than an authorization, since the agent may have `cd`'d into a sensitive dir.
- **User deny rules slot.** The unioned `permissions.deny` patterns from settings. Tells the classifier to catch
  *tool-switching circumvention* of an existing deny rule — for example, if the user has denied `Edit(/etc/**)`, the
  classifier should also block `Bash(python -c '... open()')` accomplishing the same effect. It renders as a
  soft-block bullet, not a separate section, mirroring where Auto Mode places it.
- **Session context block.** When a policy or deny rule uses a `$USER/...` branch pattern and a sanitized user identity
  is available, a short system block explains how that pattern resolves for this session.
...
| Working directories          | **Config-tier**            | The in-scope dirs the Environment block declares and the file-op skip-list trusts. `permissions.additionalDirectories` is read from trusted tiers only — the repo-controlled project `<cwd>/.claude/settings.json` is **deliberately excluded**, mirroring the `autoMode.*` exclusion: a hostile repo listing `~/.aws` / `~/.ssh` there could otherwise mark them in-scope and skip the classifier on file ops against them. `permissions.deny` is read from all tiers (extra denials only ever block more). cwd is included for the skip-list but framed to the model as a location, not an authorization. |
...
The live policy is not our own copy — we shell out to the user's installed `claude` binary at runtime, keeping a
bundled baseline (`defaults.yaml`) only as the fallback when the binary is unavailable and no valid cache exists. This
trades a one-time ~250–500 ms shell-out (cached aggressively) for zero version skew — when the user upgrades Claude
Code, our policy automatically follows. The shell-out runs from the **project root**, not the live
cwd: `claude auto-mode config` resolves project-scoped overrides (`settings.local.json`) from its own launch dir,
which doesn't follow the agent's `cd`s, so the project root is the closest stable anchor the hook exposes (see
...
When a user or default rule needs it, the rendered system prompt also includes a small Session Context block with the
sanitized user identity used to interpret `$USER/...` branch patterns.
...
flattened to `key=value` pairs (sorted). For safe helper tools that still appear in history, the projection keeps the
same compact signal Claude Code uses, such as AskUserQuestion question text or TodoWrite item counts. The projections
are modeled on Auto Mode's
...
- **XML tags** (two-stage modes): `<block>yes</block><reason>one sentence</reason>` or `<block>no</block>`, optionally
  preceded by `<thinking>...</thinking>` in stage-2 chain-of-thought calls. This is the prompt's native output
  contract.
- **Forced tool use against a JSON Schema** (`TWO_STAGE_MODE_OFF` mode): the legacy mode adapts the prompt to request
  `classify_result`, whose schema requires `thinking` before `shouldBlock` so the model has scratch space before the
  boolean.
...
back-filled with safe defaults. Final XML parse failures are treated as block verdicts, matching Auto Mode's
fail-closed posture; infrastructure failures still route through the classifier-error policy.
...
4. **Project anchor can't perfectly track Claude Code under mid-session worktree / cross-project resume.** Claude
   Code resolves its own settings (and `auto-mode config`) from a stable launch dir (`getOriginalCwd()`) the hook
   never surfaces. We anchor at the project root (`$CLAUDE_PROJECT_DIR`, else the hook cwd) — exact in the common
   cases, stable across the agent's `cd`s — but two mid-session flows move `getOriginalCwd()` away from both values
   the hook sees (`cwd`, which drifts; `$CLAUDE_PROJECT_DIR`, the project root), so we may read a different project's
   `settings.local.json` / `autoMode.*` overrides than the live session:

   - **`EnterWorktree`:** recoverable in principle (Claude Code logs an internal `worktree-state` transcript entry),
     but we don't — coupling to an undocumented entry shape isn't worth it here.
   - **Cross-project resume:** no hook-visible signal, but self-corrects — resuming a different repo has the user
     `cd` there and relaunch, so `getOriginalCwd()` becomes the new launch dir.

   Either way the blast radius is bounded — slightly-wrong *user* overrides, never a HARD BLOCK or core-policy
   bypass — so we accept it.

5. **No user-visible "thinking" indicator.** During a multi-second classifier call the user sees nothing in the
