...
match against a flat command line. That works for `git status`, but the agent rarely writes single flat commands.
A typical session is full of things like:
...
Each is benign. None match a flat allowlist pattern. Each is a permission prompt under the native model.
